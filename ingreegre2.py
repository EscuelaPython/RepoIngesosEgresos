...
class FinanzasApp:
    ...
    def crear_tablas(self):
        ...
        ''')
        # Insertar algunos bancos comunes por defecto
        cursor.execute('''
    ...
    def crear_widgets(self):
        ...
        # Banco
        ttk.Label(input_frame, text="Banco/Medio:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        self.banco_cb = ttk.Combobox(input_frame, textvariable=self.banco)
        self.banco_cb.grid(row=4, column=1, columnspan=2, sticky=tk.EW, padx=5)
        self.actualizar_bancos()
        
        # Fecha
        ttk.Label(input_frame, text="Fecha:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(input_frame, textvariable=self.fecha).grid(row=5, column=1, columnspan=2, 
                                                           sticky=tk.EW, padx=5)
        ...
        columns = ("id", "fecha", "tipo", "categoria", "banco", "descripcion", "monto")
    ...
    def agregar_movimiento(self):
        ...
        try:
            ...
            cursor = self.conn.cursor()
            cursor.execute('''
                INSERT INTO movimientos (tipo, monto, categoria, descripcion, fecha, banco)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
    ...
    def actualizar_tabla(self):
        ...
        query = "SELECT id, fecha, tipo, categoria, banco, descripcion, monto FROM movimientos"
        conditions = []
        params = []
