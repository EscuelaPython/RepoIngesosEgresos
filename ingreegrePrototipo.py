...
class AplicacionFinanzas:
    def __init__(self, root):
        ...
        self.fecha = tk.StringVar(value=datetime.now().strftime("%d-%m-%Y"))
        self.cuenta = tk.StringVar()
        self.moneda = tk.StringVar(value="Bs.")
    ...
    def configurar_interfaz(self):
        ...
        ttk.Radiobutton(registro_frame, text="Egreso", variable=self.tipo_movimiento, 
                       value="egreso", command=self.actualizar_categorias).grid(row=1, column=2, sticky=tk.W)
        
        ttk.Label(registro_frame, text="Monto:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(registro_frame, textvariable=self.monto, width=15).grid(row=0, column=1, sticky=tk.W)
        
        ttk.Label(registro_frame, text="Moneda:").grid(row=1, column=2, sticky=tk.W, padx=5)
        ttk.Combobox(registro_frame, textvariable=self.moneda, values=["Bs.", "$","USDT"], 
        ...
        self.filtro_moneda = ttk.Combobox(filtros_frame, values=["Todos", "Bs.", "$","USDT"], 
